...
该脚本会模拟输入 `d d d s s q`（右移 3 步、下移 2 步、退出）。

#### 单元测试

项目使用 **Google Test** 框架编写了单元测试，覆盖核心业务逻辑：
...
| 测试文件 | 覆盖模块 | 测试用例数 |
|----------|----------|:----------:|
| `test_entity.cpp` | `Entity`（受伤/治疗/移动）、`Monster`、`Item` | 12 |
| `test_player.cpp` | `Player`（升级/药水/武器加成） | 14 |

> **注意：** 测试会自动通过 `FetchContent` 下载 Google Test，无需手动安装。
